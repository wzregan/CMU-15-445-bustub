...
template <typename K, typename V>
ExtendibleHashTable<K, V>::Bucket::Bucket(size_t array_size, int depth, int mdi) : min_diridx(mdi), size_(array_size), depth_(depth) {
...
auto  ExtendibleHashTable<K, V>::SplitBucket(int dir_idx) -> std::shared_ptr<Bucket> {
	...
	std::shared_ptr<Bucket> new_temp2 = std::make_shared<Bucket>(2, GetGlobalDepth(), pointed_min_dir_idx + ( referenced_pointed_count - 1 ) * 4);
