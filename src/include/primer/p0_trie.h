...
public:
	...
	std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
		...
		if(children_.count(key_char) || child->GetKeyChar()!=key_char){
			return nullptr;
		}
...
class Trie {
 ...
 public:
	...
	bool Insert(const std::string &key, T value) {
		...
		for (int i = 0 ; i < (int)key.length(); i++) {
			...
			if (temp_node==nullptr){
				(*last)->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
				temp_node = (*last)->GetChildNode(ch);
			}
		}
		// 判断temp_node是否已经包含数据，如果包含则返回false，否则就将节点转换为TrieNodeWithValue
		if ((*temp_node)->IsEndNode()){
			latch_.WUnlock();
			return false;
		}else {
			TrieNode * value_node = (temp_node->release());
			(*last)->RemoveChildNode(ch);
			(*last)->InsertChildNode(ch, 
									std::make_unique<TrieNodeWithValue<T>>(std::move(*value_node),value)
									);
			delete value_node;
		}
		latch_.WUnlock();
	...
	bool Remove(const std::string &key) { 
		latch_.WLock();
		bool success = false;
		_Remove(&root_,key,0,success);
		latch_.WUnlock();
		return success ; 
	}
	bool _Remove(std::unique_ptr<bustub::TrieNode> * cur, const std::string &key, int index,bool &success){
		if (int(key.size()) == index){
			if (!(*cur)->IsEndNode())
				return false;
		...
		char ch = key[index];
		std::unique_ptr<bustub::TrieNode> * node = (*cur)->GetChildNode(ch);
		if (node==nullptr){
			return false;
		}
		bool flag = _Remove(node, key, index + 1,success);
		if (flag){
			(*cur)->RemoveChildNode(key[index]);
		}
		if((*cur)->HasChildren() || (*cur)->IsEndNode()){
			return false;
		}else{
			return true;
		}
	...
	T GetValue(const std::string &key, bool *success) {
		...
		if((*node)->IsEndNode()){
			...
			TrieNodeWithValue<T>* node_with_value = dynamic_cast<TrieNodeWithValue<T>*>(node_ori);
